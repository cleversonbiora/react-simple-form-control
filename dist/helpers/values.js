...
export function getFormValue(formId, id, values) {
    let val = "";
    if (values[formId]) if (values[formId][id]) val = values[formId][id];
    return val;
}

export function mergeValues(values) {
    let forms = Object.keys(values).map(function (e) {
        return values[e];
    });
    // eslint-disable-next-line
    let mergedValues = {};
    if (forms.length > 0) mergedValues = Object.assign(...forms);
    return mergedValues;
}
